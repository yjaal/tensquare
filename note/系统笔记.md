...
`http://10.10.10.254:12000/base-dev.yml`访问，如果能访问到配置文件则表示成功。
...
这样就可以构建，同时上传到`docker`仓库中。不过要注意：之前我们在`config`模块中为了打镜像所作的一些配置，这里也要添加到`Eureka`模块中来，同时相关配置文件需要检查`ip(10.10.10.254)`。
